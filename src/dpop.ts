 ...
 */
export function computeJwkThumbprint(jwk: Record<string, any>): string {
...
export interface DpopProofResult {
  ...
  payload: { url: string; method: string; timestamp: number };
...
export function verifyDpopProof(
  ...
  let proof: { jwk: Record<string, any>; payload: string; signature: string };
  try {
