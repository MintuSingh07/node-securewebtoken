...
exports.generateDpopKey = generateDpopKey;
exports.createDpopHeaders = createDpopHeaders;
function getCrypto() {
 ...
 */
async function createDpopHeaders(privateKey, url, method) {
    if (!privateKey) {
...
function rawToDer(rawSignatureBuffer) {
    ...
    let rOffset = 0;
    while (rOffset < r.length - 1 && r[rOffset] === 0)
        rOffset++;
    r = r.slice(rOffset);
    let sOffset = 0;
    while (sOffset < s.length - 1 && s[sOffset] === 0)
        sOffset++;
    s = s.slice(sOffset);
